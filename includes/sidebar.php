...
if(isset($_REQUEST['shop'])&&is_array($_REQUEST['shop'])){

    foreach($_REQUEST['shop'] as $sKey=>$sVal){

        if((int)$sVal!=0){

            $aShop[(int)$sVal] = (int)$sVal;
...
<input type="text" class="form-control" id="dev-table-filter" data-action="filter" data-filters="#dev-shop" placeholder="Filter Shops">
...
<ul class="nav nav-pills nav-stacked category-menu" id="dev-shop"><!-- nav nav-pills nav-stacked category-menu Starts -->

<?php

$get_shop = "select * from shops where shop_top='yes'";
...
while($row_shop = mysqli_fetch_array($run_shop)){
    ...
    if($shop_image == ""){
        ...
        <img src='admin_area/other_images/$shop_image' width='20px' >&nbsp;
    ...
    if(isset($aShop[$shop_id])){ echo "checked='checked'"; }

    echo " type='checkbox' value='$shop_id' name='shop' class='get_shop'>
...
$get_shop = "select * from shops where shop_top='no'";
